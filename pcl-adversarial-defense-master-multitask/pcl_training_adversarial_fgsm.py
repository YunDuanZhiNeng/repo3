...
parser.add_argument('--test-batch', default=100, type=int, metavar='N',
                    help='test batchsize')
parser.add_argument('--schedule', type=int, nargs='+', default=[200, 250],
...
def FGSM(model, criterion, img, label, eps):
    ...
    _, _, _, out, pre_features = model(adv)
...
def main():
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0: # and (epoch + 1) % args.eval_freq == 0 or (epoch + 1) == args.max_epoch:
            ...
            if acc>88 and epoch>args.max_epoch-150:
                ...
                torch.save(state_, 'Models_PCL_AdvTrain_FGSM/CIFAR10_PCL_AdvTrain_FGSM_'+str(epoch)+'_'+str(float(acc))+'.pth.tar')
