...
#from resnet_model_frize_full import *  # Imports the ResNet Model
import argparse
...
def attack(model, criterion, img, label, eps, attack_type, iters, out_adv_18=None):
    ...
    if attack_type == 'pgd':
        ...
        noise = 0
    yyr_index=model(normalize(img.clone().detach()))[19].argmax(dim=-1) == label
    for j in range(iterations):
        ...
        maxk=max((1,2))
        confidence=0
        pred_val, pred_id = model(normalize(adv.clone().detach()))[19].topk(maxk,1,True,True)
        ...
        mask= yyr_index.to(dtype=torch.int8)
